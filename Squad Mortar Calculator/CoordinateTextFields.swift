...
class CoordinatesTextField1: UITextField, UITextFieldDelegate {
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        if prospectiveText.count == 1 && !(prospectiveText[0].containedIn(matchCharacters: acceptableFirstCharacters)) {
            return false
        }
        if prospectiveText.count > 1 && !(prospectiveText[1].containedIn(matchCharacters: acceptableSecondCharacters)) {
            return false
        }
        if prospectiveText.count > 2 && !(prospectiveText[2].containedIn(matchCharacters: acceptableSecondCharacters)) {
            return false
        }
        return true
    }
